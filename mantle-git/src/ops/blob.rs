...
pub fn blob_oids(repo_path: &str) -> Result<HashMap<String, String>, Error> {
...
pub fn show_file(repo_path: &str, commit_hash: &str, file_path: &str) -> Result<String, Error> {
    let ts = repo::open(repo_path)?;
    let repo = ts.to_thread_local();
    ...
    let object = entry.object().map_err(Error::internal)?;
