...
pub fn init_repo(path: &str) -> Result<(), GitError> {
    git2::Repository::init(path).map_err(GitError::internal)?;
    Ok(())
}

/// Resolve the repository root (equivalent to `git rev-parse --show-toplevel`).
pub fn repo_root(path: &str) -> Result<String, GitError> {
    let repo = git2::Repository::discover(path).map_err(GitError::internal)?;
    let workdir = repo.workdir().ok_or_else(|| GitError::Internal {
...
pub fn checkout(repo_path: &str, branch: &str) -> Result<(), GitError> {
    ...
    let refname = branch_ref
        ...
        .to_owned();
    repo.set_head(&refname).map_err(GitError::internal)?;
...
pub fn branch_delete(repo_path: &str, name: &str) -> Result<(), GitError> {
    let repo = open_git2(repo_path)?;
    let mut branch = repo
...
pub fn create_branch_at(repo_path: &str, branch: &str, start_point: &str) -> Result<(), GitError> {
    let repo = open_git2(repo_path)?;
    let obj = repo
...
pub fn update_ref(repo_path: &str, refname: &str, value: &str) -> Result<(), GitError> {
    let repo = open_git2(repo_path)?;
    let oid = git2::Oid::from_str(value).map_err(GitError::internal)?;
...
pub fn delete_ref(repo_path: &str, refname: &str) -> Result<(), GitError> {
    let repo = open_git2(repo_path)?;
    let mut reference = repo.find_reference(refname).map_err(GitError::internal)?;
...
pub fn clean_untracked(repo_path: &str) -> Result<(), GitError> {
    let files = super::status::list_untracked_files(repo_path)?;
    let base = std::path::Path::new(repo_path);
    for relative in &files {
        let path = base.join(relative);
        if path.is_dir() {
            std::fs::remove_dir_all(&path).map_err(|e| GitError::Internal {
                message: format!("Failed to remove directory {}: {}", path.display(), e),
            })?;
        } else {
            std::fs::remove_file(&path).map_err(|e| GitError::Internal {
...
pub fn stash_push(repo_path: &str, message: &str) -> Result<(), GitError> {
...
pub fn stash_pop(repo_path: &str) -> Result<(), GitError> {
    let mut repo = open_git2(repo_path)?;
    repo.stash_pop(0, None).map_err(GitError::internal)?;
    Ok(())
}

/// List all stash entries (equivalent to `git stash list`).
pub fn stash_list(repo_path: &str) -> Result<Vec<crate::types::StashEntry>, GitError> {
    let mut repo = open_git2(repo_path)?;
...
pub fn stash_apply(repo_path: &str, index: u32) -> Result<(), GitError> {
    let mut repo = open_git2(repo_path)?;
...
pub fn stash_show(repo_path: &str, index: u32) -> Result<String, GitError> {
    let mut repo = open_git2(repo_path)?;
    ...
    let oid = stash_oid.ok_or_else(|| GitError::Internal {
        message: format!("stash@{{{index}}} not found"),
    })?;

    let stash_commit = repo.find_commit(oid).map_err(GitError::internal)?;
    let stash_tree = stash_commit.tree().map_err(GitError::internal)?;

    let parent = stash_commit.parent(0).map_err(GitError::internal)?;
...
pub fn add_all(repo_path: &str) -> Result<(), GitError> {
    let repo = open_git2(repo_path)?;
    let mut index = repo.index().map_err(GitError::internal)?;
    index
...
pub fn add_files(repo_path: &str, paths: &[String]) -> Result<(), GitError> {
    let repo = open_git2(repo_path)?;
    let mut index = repo.index().map_err(GitError::internal)?;
    for path in paths {
...
pub fn reset_staging(repo_path: &str) -> Result<(), GitError> {
    let repo = open_git2(repo_path)?;
    if let Ok(head) = repo.head() {
        ...
        let mut index = repo.index().map_err(GitError::internal)?;
        index.clear().map_err(GitError::internal)?;
        index.write().map_err(GitError::internal)?;
    }
    Ok(())
}
...
pub fn restore_file(repo_path: &str, file_path: &str) -> Result<(), GitError> {
    let repo = open_git2(repo_path)?;
    ...
    if has_head {
        ...
        let tree = head.peel_to_tree().map_err(GitError::internal)?;
        ...
        if tree.get_path(path).is_ok() {
            ...
            if full_path.exists() {
                std::fs::remove_file(&full_path).map_err(|e| GitError::Internal {
...
pub fn amend_commit(repo_path: &str, message: &str) -> Result<(), GitError> {
    let repo = open_git2(repo_path)?;
    let head = repo.head().map_err(GitError::internal)?;
    let head_commit = head.peel_to_commit().map_err(GitError::internal)?;
    let sig = repo.signature().map_err(GitError::internal)?;
    let mut index = repo.index().map_err(GitError::internal)?;
    let tree_oid = index.write_tree().map_err(GitError::internal)?;
...
pub fn commit(repo_path: &str, message: &str) -> Result<(), GitError> {
    ...
    let parents = match repo.head() {
        Ok(head) => {
            let parent_commit = head.peel_to_commit().map_err(GitError::internal)?;
            vec![parent_commit]
        }
